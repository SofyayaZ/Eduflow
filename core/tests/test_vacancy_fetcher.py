...
def fetcher():
    ...
    with patch('core.services.vacancy_fetcher.VacancyRepository'), \
         ...
         patch('core.services.vacancy_fetcher.VacancyCache'), \
...
class TestVacancyFetcher:
    ...
    def test_fetch_page_success(self, mock_get, fetcher):
        ...
        assert kwargs['headers']['User-Agent'] == 'EduFlow/1.0 (soniazaitceva@gmail.com)'
    ...
    def test_parse_date_invalid(self):
        # При ошибке парсинга возвращается текущее UTC время
        dt = VacancyFetcher._parse_date("invalid")
        now = datetime.now(timezone.utc)
        assert (now - dt).total_seconds() < 1  # разница меньше секунды
    ...
    def test_fetch_and_save_no_vacancies(self, mock_fetch_all, fetcher):
        mock_fetch_all.return_value = []  # для всех должностей
        result = fetcher.fetch_and_save()
        assert result == 0
    ...
    def test_fetch_and_save_new_vacancies(
        self, mock_fetch_all, mock_prereq, mock_normalizer,
        mock_extractor, mock_vacancy_skill_repo, mock_vacancy_repo, fetcher
    ):
        # Подготовка данных с правильной структурой (внутри 'vacancy')
        raw_vacancies = [
            {
                "vacancy": {
                    "id": "v1",
        ...
        mock_vacancy_repo.get_existing_ids.return_value = set()
        mock_vacancy_repo.get_or_create.side_effect = lambda **kwargs: (Mock(id=kwargs['id_vacancy']), True)
        mock_vacancy_skill_repo.get_or_create.return_value = (Mock(), True)
        ...
        assert result == 2
        mock_vacancy_repo.get_or_create.assert_called()
    ...
    def test_fetch_and_save_skip_existing(self, fetcher_with_targets):
        raw_vacancies = [{"vacancy": {"id": "v1", "job-name": "Dev", "duty": "desc"}}]
        with patch.object(VacancyFetcher, '_fetch_all_for_job_title', return_value=raw_vacancies), \
            patch('core.services.vacancy_fetcher.VacancyRepository') as mock_vacancy_repo:
            mock_vacancy_repo.get_existing_ids.return_value = {"v1"}
            mock_vacancy_repo.get_or_create.return_value = (Mock(), False)

            result = fetcher_with_targets.fetch_and_save()
            assert result == 0
            mock_vacancy_repo.get_or_create.assert_called_once()

    def test_fetch_and_save_empty_description(self, fetcher_with_targets):
        raw_vacancies = [{"vacancy": {"id": "v1", "job-name": "Dev", "duty": ""}}]  # пустое описание
        with patch.object(VacancyFetcher, '_fetch_all_for_job_title', return_value=raw_vacancies), \
            patch('core.services.vacancy_fetcher.VacancyRepository') as mock_vacancy_repo, \
            patch('core.services.vacancy_fetcher.VacancySkillRepository') as mock_vacancy_skill_repo:
